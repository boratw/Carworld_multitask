...
LOG_DIR = "data/dqn_ns1/"
log_file = open(LOG_DIR + "log.txt", "wt")

sess = tf.Session()
saver = tf.train.Saver(max_to_keep=0)
log_file.write("Episode\tScore\tStep\tQvalue\tQvalue_Std\tQvalue_Loss\tState_Loss\n")
...
with sess.as_default():
    ...
    for episode in range(1, 5001):
        ...
        log_file.write(str(episode) + "\t" +
            str(cur_reward / cur_batch) + "\t" + str(cur_step / cur_success_batch if cur_success_batch != 0 else 0.) +  "\t" + str(q1) + "\t" + str(qs1) + "\t" + str(ql1) + "\t" + str(sl1) + "\n")
        if episode % 200 == 0:
