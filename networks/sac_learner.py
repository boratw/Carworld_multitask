...
class SACLearner:
    def __init__(self, state_len, action_len, name="", 
        ...
        with tf.variable_scope("SACLearner" + name): 
            ...
            input_action_ = self.input_action * self.action_limit
            ...
            self.qvalue1_network = MLP("qvalue1", state_len, 1, qvalue_hidden_len, hidden_nonlinearity=tf.nn.leaky_relu,
                input_tensor=self.input_state, additional_input=True, additional_input_dim=action_len, additional_input_tensor=input_action_)
            ...
            self.policy_network = GaussianPolicy("policy", state_len, action_len, input_tensor=self.input_state,
                hidden_nonlinearity=tf.nn.tanh, output_nonlinearity=tf.nn.tanh, action_limit=self.action_limit)
            ...
            self.qvalue1_network_forpolicy = MLP("qvalue1", state_len, 1, qvalue_hidden_len, hidden_nonlinearity=tf.nn.leaky_relu, reuse=True,
                ...
                additional_input_tensor=self.policy_network.x)
            ...
            final_soft_value = tf.maximum(soft_value, self.input_value)
            self.qvalue1_loss = tf.reduce_mean((final_soft_value - self.qvalue1_network.layer_output) ** 2)
            ...
            self.qvalue2_loss = tf.reduce_mean((final_soft_value - self.qvalue2_network.layer_output) ** 2)
            ...
            #self.policy_loss = tf.reduce_mean((self.policy_network.log_pi * tf.stop_gradient(self.policy_network.log_pi - self.qvalue1_network_forpolicy.layer_output + self.value_network.layer_output)) )
            ...
            #self.policy_likelihood = self.policy_network.log_li(input_action_)
            policy_prior = tf.contrib.distributions.MultivariateNormalDiag(loc=tf.zeros(action_len), scale_diag=tf.ones(action_len))
            policy_prior_log_probs = policy_prior.log_prob(self.policy_network.x)

            min_log_target = tf.minimum(self.qvalue1_network_forpolicy.layer_output, self.qvalue2_network_forpolicy.layer_output)
            self.value_loss = tf.reduce_mean((self.value_network.layer_output - tf.stop_gradient(min_log_target - self.policy_network.log_pi + policy_prior_log_probs)) ** 2) 
    ...
    def get_action(self, input_state):
        ...
        return output / self.action_limit

    def get_action_walk(self, input_state):
        sess = tf.get_default_session()
        output = sess.run(self.policy_network.walk, {self.input_state : np.array([input_state])})
        return output / self.action_limit
