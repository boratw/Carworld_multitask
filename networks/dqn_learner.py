...
class DQNLearner:
    def __init__(self, state_len, action_len, name="", hidden_len=256, qvalue_lr=0.001, gamma=0.96):
        self.gamma = gamma
        ...
        with tf.variable_scope("DQNLearner" + name): 
            ...
            self.next_network = DQNNetwork("main", state_len, action_len, hidden_len, reuse=True, input_tensor=self.input_next_state)
    ...
    def get_action_random_with_qvalue(self, input_state, random_scale):
        ...
        rnd = np.random.normal(0.0, random_scale, size=output.shape)
        output += rnd
        return np.argmax(output), np.max(output[0])
    ...
    def optimize(self, input_state, input_next_state, input_action, input_reward, input_survive, input_value):
        ...
        sess = tf.get_default_session()
        q, ql, _ = sess.run([self.main_network.output_qvalue, self.qvalue_loss, self.qvalue_train], input_list)
        q = q.reshape(-1)
